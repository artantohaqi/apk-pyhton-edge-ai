...
class AIEngine:
    ...
    # ==========================================================
    # BARU: process_batch()
    # Wristband sekarang publish 1x per detik berisi ARRAY sample
    # (hasil batching 60Hz di firmware), bukan 1 sample per publish
    # lagi. Method ini menerima payload MQTT APA ADANYA dan
    # meng-iterasi tiap sample secara berurutan lewat
    # process_raw_signals(), supaya urutan sample & asumsi jarak
    # waktu antar-sample (16.67ms) tetap valid untuk deteksi peak.
    # ==========================================================
    def process_batch(self, payload: dict):
        """
        payload = {"data": [{"ir":.., "red":.., "ax":.., "ay":.., "az":..,
                              "gx":.., "gy":.., "gz":..}, ...], "count": N}
        Return: (current_hr, current_rmssd, motion_level) dari sample TERAKHIR
                di batch, atau None kalau batch kosong / semua motion tinggi.
        """
        ...
        for s in samples:
            hr, ibi, _ = self.process_raw_signals(
                s.get("ir", 0), s.get("ax", 0), s.get("ay", 0), s.get("az", 0)
            )
            last_ax, last_ay, last_az = s.get("ax", 0), s.get("ay", 0), s.get("az", 0)
            # red, gx, gy, gz sengaja belum dipakai -- lihat catatan di bawah

        current_hr = self.last_hr
